...
public class DynamicListView extends ListView {
	...
	private ArrayList<Exercise>list_exercise=new ArrayList<>();
	...
	public static ArrayList<View>View_List=new ArrayList<>();;
	...
	public void setExercise12(ArrayList<Exercise>ex){
		this.list_exercise=ex;
	...
	public void onGrab(int position, RelativeLayout selectedView) {
		mTotalOffset = 0;
		mMobileItemId = MainActivity.listviewadapter.getItemId(position);
	...
	private OnItemLongClickListener OnItemLongClickListener=new OnItemLongClickListener() {
		...
		public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
			view.setBackgroundColor(Color.rgb(165,193,239));
			Log.e("main","position"+position+",,"+id+list_exercise);
			position_list=position;

			final AlertDialog.Builder delete=new AlertDialog.Builder(mContext).setTitle("삭제").setMessage("선택한 운동("+list_exercise.get(position).getTitle()+")을 삭제하시겠습니까?").setPositiveButton("네", new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {

					Log.e("main",",,,,,,!!!!!"+list_exercise);
					MainActivity.db.execSQL("delete from exercise where exer_id="+list_exercise.get(position_list).getExer_id());
					list_exercise.remove(position_list);
					MainActivity.listviewadapter.setmIdMap(list_exercise);
					MainActivity.listviewadapter.notifyDataSetChanged();
					Log.e("main","남은 리스트 목록 : "+list_exercise);
	...
	private OnItemClickListener OnItemClickListener= new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			Log.e("main",position+",,,"+id+",,,,,");
			background= view.getBackground();
			if(background instanceof ColorDrawable){
				color=((ColorDrawable) background).getColor();
				Log.e("main", "color : "+color+",,,"+Color.GRAY);

			}
			if(color==Color.rgb(165,193,239)){
				list_exercise.get(position).setSelected(false);
				Log.e("main","gray므로 화이트로바꿈"+position+",,,,"+list_exercise.get(position));
				view.setBackgroundColor(Color.WHITE);
			}else{
				list_exercise.get(position).setSelected(true);

				View_List.add(view);

				Log.e("main","white to grey로바꿈");
				view.setBackgroundColor(Color.rgb(165,193,239));
			}
			Log.e("main","전체 엑서 리스트는 : "+list_exercise);
			Log.e("main",View_List.size()+"zmrl");


		}
	};
	public void SetView_List(){
		Log.e("main","바꿀 뷰 사이즈는 : "+View_List.size());
	...
	private void updateNeighborViewsForID(long itemID) {
		...
		mAboveItemId = MainActivity.listviewadapter.getItemId(position - 1);
		mBelowItemId = MainActivity.listviewadapter.getItemId(position + 1);
	}
	...
	public View getViewForID (long itemID) {
		...
		for(int i = 0; i < getChildCount(); i++) {
			...
			long id = MainActivity.listviewadapter.getItemId(position);
			if (id == itemID) {
				return v;
			}
		}
